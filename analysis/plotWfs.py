...
# kwant.plotter.map(self.system, np.absolute(self.getWaveFunction(lead_id, energy)[0])**2, oversampling=10, cmap=cmap)
...
# plt.savefig('WF_K_prime.pdf')

# fig, ax = plt.subplots(1,1, figsize=(20, 10))
# best_structure.visualizeSystem(args={'ax': ax})
# stuff_before = ax.get_children()

# im = kwant.plotter.map(self.system, np.absolute(self.getWaveFunction(lead_id, energy)[0])**2, oversampling=10, cmap=cmap)
