...
def getMaxCurrentsForFile(filename):
    ...
    for i in range(n_generations):
        ...
        objs = np.abs(gen_data[:,0] / (gen_data[:,0] + gen_data[:,1])) + np.abs(gen_data[:,3] / (gen_data[:,2] + gen_data[:,3])) + (gen_data[:,0] + gen_data[:,3]) / 0.3
...
objectives_max = np.max(all_objs, axis=0)
purities_max = np.max(all_ps, axis=0)
currents_max = np.max(all_cs, axis=0)
...
axes_left.plot(range(n_generations), objectives, color=cm.deep(0.5), alpha=1.0, lw=5.0, label='Mean')
axes_left.plot(range(n_generations), objectives_max, '--', color=cm.deep(0.5), alpha=1.0, lw=5.0, label='Max')
...
axes_middle_top.plot(range(n_generations), purities[:,0], color=cm.deep(0.25), alpha=1.0, lw=5.0, label='Mean $K\'$')
axes_middle_top.plot(range(n_generations), purities_max[:,0], '--', color=cm.deep(0.25), alpha=1.0, lw=5.0, label='Max $K\'$')
...
axes_middle_bottom.plot(range(n_generations), purities[:,1], color=cm.deep(0.75), alpha=1.0, lw=5.0, label='Mean $K$')
axes_middle_bottom.plot(range(n_generations), purities_max[:,1],'--', color=cm.deep(0.75), alpha=1.0, lw=5.0, label='Max $K$')
...
axes_middle_top.set_ylim([0.7, 1.0])
axes_middle_bottom.set_ylim([0.7, 1.0])
...
axes_right_0.plot(range(n_generations), currents[:,1], '--', color=cm.deep(0.25), alpha=1.0, lw=5.0, label='lead 2: $K\'$')
# axes_right_1.set_ylim([0.0, 0.01])
axes_right_0.fill_between(range(n_generations), currents[:,1] - currents_stds[:,1], currents[:,1] + currents_stds[:,1], color=cm.deep(0.25), alpha=0.2)
plt.setp(axes_right_0.get_xticklabels(), visible=False)

# axes[2].plot(range(n_generations), expRunningAvg(currents[:,2]), 'r--', lw=5.0, label='lead 2: $k\'$')
axes_right_1.plot(range(n_generations), currents[:,2], color=cm.deep(0.75), alpha=1.0, lw=5.0, label='lead 1: $K$')
axes_right_1.fill_between(range(n_generations), currents[:,2] - currents_stds[:,2], currents[:,2] + currents_stds[:,2], color=cm.deep(0.75), alpha=0.2)
plt.setp(axes_right_1.get_xticklabels(), visible=True)
# axes_right_2.set_ylim([0.02, 0.12])


axes_right_1.plot(range(n_generations), currents[:,3], '--', color=cm.deep(0.75), alpha=1.0, lw=5.0, label='lead 2: $K$')
axes_right_1.fill_between(range(n_generations), currents[:,3] - currents_stds[:,3], currents[:,3] + currents_stds[:,3], color=cm.deep(0.75), alpha=0.2)
